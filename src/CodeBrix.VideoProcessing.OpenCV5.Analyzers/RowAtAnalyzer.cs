...
namespace CodeBrix.VideoProcessing.OpenCV5.Analyzers; //was previously: OpenCvSharp.Analyzers;
...
public sealed class RowAtAnalyzer : DiagnosticAnalyzer
{
    ...
    /// <summary>
    /// Returns true when <paramref name="expr"/> is a call to <c>CodeBrix.VideoProcessing.OpenCV5.Mat.At&lt;T&gt;(int)</c>.
    /// </summary>
    private static bool IsMatAtIntCall(InvocationExpressionSyntax expr, SemanticModel model)
    {
        ...
        return sym is { Parameters.Length: 1 } &&
               sym.ContainingType?.ToDisplayString() == "CodeBrix.VideoProcessing.OpenCV5.Mat" &&
               sym.Parameters[0].Type.SpecialType == SpecialType.System_Int32;
    }

    /// <summary>
    /// Returns true when <paramref name="expr"/> is a call to <c>CodeBrix.VideoProcessing.OpenCV5.Mat.Row(int)</c>.
    /// </summary>
    private static bool IsMatRowCall(ExpressionSyntax expr, SemanticModel model)
    {
        ...
        return sym?.ContainingType?.ToDisplayString() == "CodeBrix.VideoProcessing.OpenCV5.Mat";
    }
