...
public class BOWKMeansTrainer : BOWTrainer
{
        ...
        int attempts = 3, KMeansFlags flags = KMeansFlags.PpCenters)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods.xfeatures2d_BOWKMeansTrainer_new(clusterCount, termCritValue, attempts, (int)flags, out var p));
        InitSafeHandle(p);
    }
    ...
    private void InitSafeHandle(IntPtr p, bool ownsHandle = true)
    {
        SetSafeHandle(new OpenCvPtrSafeHandle(p, ownsHandle,
            static h => NativeMethods.HandleException(NativeMethods.xfeatures2d_BOWKMeansTrainer_delete(h))));
    }
